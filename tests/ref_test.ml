...
let ref_test1 _ =
  ...
  let id_ty_vals, _, _, _, _ = read_string_eval "counter ();;" env tyenv tylenv varenv in
...
let ref_test2 _ =
  let env, tyenv, tylenv, varenv = initial_env, initial_tyenv, initial_tyvenv, initial_varenv in
  let _, env, tyenv, _, _ = read_string_eval "let f = ref (fun x -> x);;" env tyenv tylenv varenv in
  let id_ty_vals, env, tyenv, _, _ = read_string_eval "!f 0;;" env tyenv tylenv varenv in
