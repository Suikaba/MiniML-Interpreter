...
let let_poly_test1 _ =
  ...
  let id_ty_vals, _, _, _, _ = read_string_eval "apply_three_times (fun x -> x * 2) 2;;" env tyenv tylenv varenv in
  assert_equal id_ty_vals [("-", TyInt, IntV 16)];
  let id_ty_vals, _, _, _, _ = read_string_eval "apply_three_times not true;;" env tyenv tylenv varenv in
