...
let variant_test1 _ =
  ...
  let _, _, _, tylenv, varenv, _ = read_string_eval expr env tyenv tylenv varenv [] in
  let id_ty_vals, _, _, _, _, _ = read_string_eval "A 1;;" env tyenv tylenv varenv [] in
  assert_equal id_ty_vals [("-", TyVariant "exp", VariantV ("A", IntV 1))];
  let id_ty_vals, _, _, _, _, _ = read_string_eval "B true;;" env tyenv tylenv varenv [] in
...
let variant_test2 _ = (* same tag variant *)
  ...
  let _, _, _, tylenv, varenv, _ = read_string_eval expr env tyenv tylenv varenv [] in
  let expr = "type exp2 = A of int | C;;" in
  let _, _, _, tylenv, varenv, _ = read_string_eval expr env tyenv tylenv varenv [] in
  let id_ty_vals, _, _, _, _, _ = read_string_eval "A 1;;" env tyenv tylenv varenv [] in
  assert_equal id_ty_vals [("-", TyVariant "exp2", VariantV ("A", IntV 1))];
  let id_ty_vals, _, _, _, _, _ = read_string_eval "C;;" env tyenv tylenv varenv [] in
  assert_equal id_ty_vals [("-", TyVariant "exp2", VariantV ("C", UnitV))]
