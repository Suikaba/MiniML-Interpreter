...
let matching_test3 _ =
  ...
  let id_ty_vals, _, _, _, _, _ = read_string_eval "f (A 0);;" env tyenv tylenv varenv [] in
  assert_equal id_ty_vals [("-", TyInt, IntV 1)];
  let id_ty_vals, _, _, _, _, _ = read_string_eval "f (B false);;" env tyenv tylenv varenv [] in
  assert_equal id_ty_vals [("-", TyInt, IntV 2)];
